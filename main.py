...
def print_database_to_stderr():
    """Print current database state to stderr for human inspection."""
    try:
        ...
        # Format and print thoughts table
        print("\n=== THOUGHTS TABLE ===", file=sys.stderr)
        print("ID | AGENT | TIMESTAMP | CONTENT (truncated) | STATUS", file=sys.stderr)
        for t in thoughts:
            content = t.content[:20] + "..." if len(t.content) > 20 else t.content
            print(f"{t.id[:8]}| {t.agent_signature[:10]}| {t.created_at}| {content}| {t.status}", 
                file=sys.stderr)
        
        # Format and print plans table
        print("\n=== PLANS TABLE ===", file=sys.stderr)
        print("ID | AGENT | TIMESTAMP | TITLE | VERSION | STATUS", file=sys.stderr)
        for p in plans:
            print(f"{p.id[:8]}| {p.agent_signature[:10]}| {p.created_at}| {p.title[:20]}| {p.version}| {p.status}", 
                file=sys.stderr)
        
        # Format and print changes table
        print("\n=== CHANGES TABLE ===", file=sys.stderr)
        print("ID | AGENT | TIMESTAMP | PLAN_ID | DESCRIPTION (truncated)", file=sys.stderr)
        for c in changes:
            desc = c.description[:20] + "..." if len(c.description) > 20 else c.description
            print(f"{c.id[:8]}| {c.agent_signature[:10]}| {c.created_at}| {c.plan_id[:8]}| {desc}", 
                file=sys.stderr)
        
        session.close()
    except Exception as e:
        print(f"Error printing database: {str(e)}", file=sys.stderr)
...
# Web Dashboard route
@mcp.resource(path="/dashboard", methods=["GET"])
async def dashboard():
    """Simple HTML dashboard for human database inspection."""
    session = SessionLocal()
    ...
    <html>
    <head>
        <title>TPC Server Dashboard</title>
        <style>
    ...
    <body>
        <h1>Thoughts, Plans and Changelog Server Dashboard</h1>
        <p>Last updated: {datetime.utcnow().isoformat()}</p>
        
        <h2>Thoughts ({len(thoughts)})</h2>
        <table>
            <tr>
                <th>ID</th>
                <th>Agent</th>
    ...
    for thought in thoughts:
        html_content += f"""
            <tr>
                <td>{thought.id}</td>
                <td>{thought.agent_signature}</td>
                <td>{thought.created_at}</td>
                <td class="truncate">{thought.content}</td>
                <td>{thought.status}</td>
            </tr>
        """
    ...
    for plan in plans:
        html_content += f"""
            <tr>
                <td>{plan.id}</td>
                <td>{plan.agent_signature}</td>
                <td>{plan.created_at}</td>
                <td>{plan.title}</td>
                <td class="truncate">{plan.description}</td>
                <td>{plan.version}</td>
                <td>{plan.status}</td>
            </tr>
        """
    ...
    for change in changes:
        html_content += f"""
            <tr>
                <td>{change.id}</td>
                <td>{change.agent_signature}</td>
                <td>{change.created_at}</td>
                <td>{change.plan_id}</td>
                <td class="truncate">{change.description}</td>
            </tr>
        """
